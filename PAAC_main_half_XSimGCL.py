...
class PAAC(torch.nn.Module):
    def __init__(self, config, data):
        ...
        self.keep_layer = config.keep_layer
        # data
    ...
    def batch_loss(self, u_idx, i_idx, j_idx):
        ...
        user_embedding, item_embedding, first_view, final_view = self.forward(perturbed=False)
        _, _, first_view, final_view =  self.forward(perturbed=True)
        ...
        cl_loss, user_cl_loss, item_cl_loss = self.xcl_loss(first_view, final_view, u_idx, i_idx, j_idx)
        batch_loss = bpr_loss + l2_loss + cl_loss
        return batch_loss, bpr_loss, l2_loss, cl_loss, user_cl_loss, item_cl_loss


def test(model):
    user_embedding, item_embedding, _, _ = model.forward(perturbed=False)
...
def train(config, data, model, optimizer, early_stopping, logger, train_step=1):
    ...
    for epoch in range(config.num_epoch):
        ...
        user_emb, item_emb, _, _ = model.forward(perturbed=False)
                ...
                for temperature in ast.literal_eval(config.temperature_list):
                    for lambda2 in ast.literal_eval(config.lambada_list):
                        for gamma in ast.literal_eval(config.gama_list):
                            for keep_layer in ast.literal_eval(config.keep_layers_list):
                                f = open('/'.join(
                                    (config.result_path, config.model, config.dataset_name)) + '/best_performace.txt',
                                         'a+')
