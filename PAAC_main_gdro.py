...
def main_args():
    ...
    # DRO Hyperparameters
    args.add_argument('--n_groups', default=5, type=int)
    args.add_argument('--dro_temperature', default=0.2, type=float)
...
class PAAC(torch.nn.Module):
    def __init__(self, config, data):
        ...
        # DRO parameters
        self.n_groups = getattr(config, 'n_groups', 5)
        self.dro_temperature = getattr(config, 'dro_temperature', 0.2)
        
        # Latent centroids for DRO
        self.latent_centroids = torch.nn.Parameter(torch.empty(self.n_groups, self.emb_size))
        torch.nn.init.xavier_uniform_(self.latent_centroids)
    ...
    def bpr_loss(self, user_emb, pos_emb, neg_emb):
        ...
        # Return vector loss [batch_size]
        bpr_loss_vec = -torch.log(10e-8 + torch.sigmoid(pos_score - neg_score))

        l2_loss = torch.norm(user_emb, p=2) + torch.norm(pos_emb, p=2)
        l2_loss = self.decay * l2_loss

        return bpr_loss_vec, l2_loss

    def calculate_dro_loss(self, per_sample_loss, sample_representation):
        """
        Calculate Non-Adversarial Latent Group DRO loss.
        
        Args:
            per_sample_loss: [Batch] - Vector of BPR losses for each sample
            sample_representation: [Batch, emb_size] - Feature used for clustering (e.g., user_emb * pos_emb)
            
        Returns:
            dro_loss: Scalar tensor
        """
        # 1. Similarity logits: [Batch, n_groups]
        logits = torch.matmul(sample_representation, self.latent_centroids.t())
        
        # 2. Assignment weights W (Softmax): [Batch, n_groups]
        probs = F.softmax(logits, dim=1)
        
        # 3. Group losses L_g
        # weighted_losses: [Batch, n_groups]
        weighted_losses = probs * per_sample_loss.view(-1, 1)
        
        # group_counts: [n_groups] (soft count)
        group_counts = probs.sum(dim=0)
        
        # group_losses: [n_groups]
        group_losses = weighted_losses.sum(dim=0) / (group_counts + 1e-10)
        
        # 4. Final LSE loss
        # Formula: (1/eta) * ln( (1/K) * sum( exp(eta * L_g) ) )
        #        = (1/eta) * ( logsumexp(eta * L_g) - ln(K) )
        
        eta = self.dro_temperature
        lse_term = torch.logsumexp(eta * group_losses, dim=0)
        dro_loss = (lse_term - torch.log(torch.tensor(self.n_groups, dtype=torch.float, device=self.device))) / eta
    ...
    def batch_loss(self, u_idx, i_idx, j_idx):
        ...
        # 1. Get per-sample BPR loss (vector) and L2 loss (scalar)
        bpr_loss_vec, l2_loss = self.bpr_loss(user_emb, pos_emb, neg_emb)
        
        # 2. Calculate sample representation for clustering
        # Using Hadamard product of user and item embeddings
        sample_representation = torch.mul(user_emb, pos_emb)
        
        # 3. Calculate DRO loss
        dro_loss = self.calculate_dro_loss(bpr_loss_vec, sample_representation.detach())
        
        # 4. Calculate CL loss
        cl_loss, user_cl_loss, item_cl_loss = self.cl_loss(u_idx, i_idx, j_idx)
        
        # 5. Total loss
        batch_loss = dro_loss + l2_loss + cl_loss
        
        # Return mean of BPR loss for logging compatibility
        return batch_loss, bpr_loss_vec.mean(), l2_loss, cl_loss, user_cl_loss, item_cl_loss
