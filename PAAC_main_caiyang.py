...
def train(config, data, model, optimizer, early_stopping, logger, train_step=1):
    ...
    for epoch in range(config.num_epoch):
        ...
        with tqdm(total=math.ceil(len(data.training_data) / config.batch_size), desc=f'Epoch {epoch}',
                  unit='batch') as pbar:
            # Use the new popularity-aware negative sampling
            for n, batch in enumerate(dataloader.next_batch_pairwise_pop_aware(data, config.batch_size, model)):
        ...
        logger.info(
            'val_hr@20:{:.6f}   val_recall@20:{:.6f}   val_ndcg@20:{:.6f}   train_time:{}s   test_tiem:{}s'.format(
                val_hr, val_recall, val_ndcg, (trin_time - start).seconds,
                (datetime.datetime.now() - trin_time).seconds))
                
        # Val Pop/Unpop Split Evaluation
