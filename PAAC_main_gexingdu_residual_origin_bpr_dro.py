...
def main_args():
    ...
    args.add_argument('--dro_rate_list', default='[0.5]', type=str)
    ...
    args.add_argument('--pop_scale_list', default='[0.5]', type=str)
...
class PAAC(torch.nn.Module):
    def __init__(self, config, data):
        ...
        self.pop_scale = config.pop_scale
        self.dro_rate = config.dro_rate
    ...
    def bpr_loss(self, user_emb, pos_emb, neg_emb, pos_idx, neg_idx):
        ...
        origin_pos_score = self.pop_scale * pos_int_dot
        origin_neg_score = self.pop_scale * neg_int_dot

        pos_score = (1-self.pop_scale) * pos_score + origin_pos_score
        neg_score = (1-self.pop_scale) * neg_score + origin_neg_score

        bpr_loss = -torch.log(10e-8 + torch.sigmoid(pos_score - neg_score))
        if self.dro_rate==0.0:
            # print("zou 0")
            dro_loss = torch.tensor(0.0).to(self.device)
        else:
            dro_loss = self.dro_rate * bpr_loss.std()
        l2_loss = self.decay * (user_emb.norm(2) + pos_emb.norm(2) + neg_emb.norm(2) + self.W_pop.norm(2))
        return bpr_loss.mean(), l2_loss, dro_loss
    ...
    def batch_loss(self, u_idx, i_idx, j_idx):
        ...
        bpr_loss, l2_loss, dro_loss = self.bpr_loss(user_emb, pos_emb, neg_emb, i_idx, j_idx)
        cl_loss, user_cl_loss, item_cl_loss = self.cl_loss(u_idx, i_idx, j_idx)
        batch_loss = bpr_loss + l2_loss + cl_loss + dro_loss
                        ...
                        for gamma in ast.literal_eval(config.gama_list):
                            for tau in ast.literal_eval(config.tau_list):
                                for pop_gamma in ast.literal_eval(config.pop_gamma_list):
                                    for pop_scale in ast.literal_eval(config.pop_scale_list):
                                        for dro_rate in ast.literal_eval(config.dro_rate_list):
                                            f = open('/'.join((config.result_path, config.model, config.dataset_name)) + '/best_performace.txt', 'a+')
                                            f.write("PAAC_main_gexingdu_residual_origin_bpr_dro")
