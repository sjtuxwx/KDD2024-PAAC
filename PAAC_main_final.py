...
def main_args():
    # model description
    args = argparse.ArgumentParser(description="PAAC_Final_V2")
    ...
    # LightGCN model
    args.add_argument('--model', default='PAAC_Final_V2', type=str)
...
class PAAC(torch.nn.Module):
    ...
    def bpr_loss(self, user_emb, pos_emb, neg_emb, pos_idx, neg_idx):
        ...
        # === 升级版 BPR: 引入论文中的 Hard Negative Weighting ===
        # 逻辑：如果 Neg 样本比 Pos 样本更热门，说明它是 "Popularity Hard Negative"，加权惩罚
        weight = torch.ones_like(pos_score)
        # 找出 neg_f > pos_f 的样本
        hard_neg_mask = neg_f > pos_f
        weight[hard_neg_mask] = self.hard_neg_weight  # 默认 1.2

        # 计算加权 BPR Loss
        loss_val = -torch.log(10e-8 + torch.sigmoid(pos_score - neg_score))
        bpr_loss = (loss_val * weight).mean()
    ...
    # === 关键修正: 基于真实行为的群体对齐 (Oracle Group Alignment) ===
    # 这比基于预测的 Soft Alignment 稳定得多
    def get_oracle_align_loss(self, user_emb, pos_idx):
        # 1. 获取当前 Batch 用户点击的物品真实热度
        batch_pop = self.pop_count_tensor[pos_idx]

        # 2. 根据中位数将用户分为两组: High-Pop Group 和 Low-Pop Group
        #    这基于 Ground Truth，不会出错
        median_pop = batch_pop.median()
        is_high = batch_pop > median_pop

        # 3. 分组 Embedding
        users_high = user_emb[is_high]
        users_low = user_emb[~is_high]

        align_loss = 0.0

        # 4. 计算组内紧凑度 (Intra-class Compactness)
        #    让 High 组的用户靠近 High 组中心，Low 组靠近 Low 组中心
        if users_high.shape[0] > 1:
            center_high = users_high.mean(dim=0, keepdim=True).detach()  # Detach center to stabilize
            # 使用 Cosine Distance: 1 - Cosine
            high_norm = F.normalize(users_high, dim=1)
            center_high_norm = F.normalize(center_high, dim=1)
            # 距离越小越好
            dist_high = 1.0 - torch.mm(high_norm, center_high_norm.t())
            align_loss += dist_high.mean()

        if users_low.shape[0] > 1:
            center_low = users_low.mean(dim=0, keepdim=True).detach()
            low_norm = F.normalize(users_low, dim=1)
            center_low_norm = F.normalize(center_low, dim=1)
            dist_low = 1.0 - torch.mm(low_norm, center_low_norm.t())
            align_loss += dist_low.mean()
    ...
    def batch_loss(self, u_idx, i_idx, j_idx):
        ...
        # 1. BPR Loss (带 Hard Neg Weighting)
        ...
        # 3. User Alignment Loss (基于 Ground Truth)
        #    不再依赖不稳定的 W_pop 预测
        align_loss = self.get_oracle_align_loss(user_emb, i_idx)
