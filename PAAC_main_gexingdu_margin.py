...
def main_args():
    ...
    # [NEW] BUAM Parameters for Hyperparameter Search
    # margin_coeff: 控制 Margin 的整体力度 (建议范围 0.1 ~ 0.5)
    args.add_argument('--margin_coeff_list', default='[0.2]', type=str)
    # margin_gamma: 控制对冷门的敏感度衰减 (建议范围 0.2 ~ 0.5)
    args.add_argument('--margin_gamma_list', default='[0.25]', type=str)
...
class PAAC(torch.nn.Module):
    def __init__(self, config, data):
        ...
        # [NEW] BUAM Parameters
        self.margin_coeff = getattr(config, 'margin_coeff', 0.2)
        self.margin_gamma = getattr(config, 'margin_gamma', 0.25)
    ...
    def bpr_loss(self, user_emb, pos_emb, neg_emb, pos_idx, neg_idx):
        # 1. Interest Score (LightGCN) - 基础得分
        ...
        # 2. Popularity Normalization + Smoothing (PAAC 原有逻辑)
        ...
        # 3. Bilinear Sensitivity beta (PAAC 原有逻辑)
        ...
        # 4. Gaussian Kernel Adaptation M_pop (PAAC 原有逻辑)
        pos_m_pop = torch.exp(-torch.pow(pos_p - pos_beta, 2) / self.tau)
        neg_m_pop = torch.exp(-torch.pow(neg_p - neg_beta, 2) / self.tau)

        # 5. Final Score (推理/打分时的逻辑，PAAC 负责)
        pos_score = pos_interest * (self.w1 * pos_m_pop)
        neg_score = neg_interest * (self.w1 * neg_m_pop)

        # ================= [BUAM INNOVATION START] =================
        # 创新点：双边不确定性自适应 Margin (Bilateral Uncertainty-Aware Adaptive Margin)
        # 逻辑：训练阶段的梯度纠偏。
        # 越冷门的物品 (Pop 越小)，不确定性越大 (1/Pop^gamma 越大)，需要的 Margin 越大。
        
        # 归一化流行度 (防止数值过大)
        pos_norm = pos_f / self.max_pop
        neg_norm = neg_f / self.max_pop
        
        # 计算不确定性项 (Uncertainty Term)
        # 加上 1e-6 防止除以 0
        uncert_pos = 1.0 / (torch.pow(pos_norm, self.margin_gamma) + 1e-6)
        uncert_neg = 1.0 / (torch.pow(neg_norm, self.margin_gamma) + 1e-6)
        
        # 组合 Margin (双边相加)
        # 如果两个都是热门，uncert都很小，margin -> 0 (退化为 BPR)
        # 如果有冷门，margin 变大，强迫模型拉开距离
        adaptive_margin = self.margin_coeff * (uncert_pos + uncert_neg)
        
        # 计算 Loss：Score 差值必须大于 Margin
        bpr_loss = -torch.log(10e-8 + torch.sigmoid(pos_score - neg_score - adaptive_margin)).mean()
        # ================= [BUAM INNOVATION END] ===================
...
def main(config):
    ...
    # [NEW] 修改文件名，加入 m_coeff 和 m_gamma，方便区分实验结果
    file_name = '_'.join((str(config.layers), str(config.cl_rate), str(config.align_reg), str(config.gamma),
                          str(config.lambda2), "m_co"+str(config.margin_coeff), "m_ga"+str(config.margin_gamma), timestamp))
                        ...
                        for gamma in ast.literal_eval(config.gama_list):
                            for tau in ast.literal_eval(config.tau_list):
                                for pop_gamma in ast.literal_eval(config.pop_gamma_list):
                                    # [NEW] 增加 BUAM 参数循环
                                    for margin_coeff in ast.literal_eval(config.margin_coeff_list):
                                        for margin_gamma in ast.literal_eval(config.margin_gamma_list):
                                            f = open('/'.join((config.result_path, config.model, config.dataset_name)) + '/best_performace.txt', 'a+')
                                            f.write("PAAC_main_gexingdu_margin")
                                            ...
                                            config.tau=tau
                                            # [NEW] 设置新的超参
                                            config.margin_coeff = margin_coeff
                                            config.margin_gamma = margin_gamma
                                            ...
                                            val_hr, val_recall, val_ndcg, test_OOD_hr, test_OOD_recall, test_OOD_ndcg, test_IID_hr, test_IID_recall, test_IID_ndcg, result_path = main(
                                                config)
                                            f.write('\n')
                                            # [NEW] Log 中增加 margin_coeff 和 margin_gamma 的记录
                                            f.write(
                                                '\n ====layers:{}===cl-rate:{}===align_reg:{}===gamma:{}====lambda2:{}====tau:{}====pop_gamma:{}====margin_coeff:{}====margin_gamma:{}====\n  best_hr@20:{}=====best_recall@20:{}====best_ndcg@20:{}\n test_OOD_hr@20:{:.6f}   test_OOD_recall@20:{:.6f}   test_OOD_ndcg@20:{:.6f}\n test_IID_hr@20:{:.6f}   test_IID_recall@20:{:.6f}   test_IID_ndcg@20:{:.6f} \n  Resulst_path:{}\n '
                                                .format(config.layers, config.cl_rate, config.align_reg, config.gamma, config.lambda2, config.tau, config.pop_gamma, config.margin_coeff, config.margin_gamma,
