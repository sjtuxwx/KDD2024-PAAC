...
def main_args():
    # model description
    args = argparse.ArgumentParser(description="PAAC_PGSM")
    ...
    # LightGCN model
    args.add_argument('--model', default='PAAC_PGSM', type=str)
...
class PAAC(torch.nn.Module):
    def __init__(self, config, data):
        ...
        # === 模块 2: 个性化图谱调制网络 (PGSM - 新增核心) ===
        # 输入: User Embedding (emb_size)
        # 输出: K+1 个层的权重 (layers + 1)
        # 作用: 作为一个带通滤波器，决定用户接受哪一层的信息更多
        self.spectrum_gate = torch.nn.Sequential(
            torch.nn.Linear(self.emb_size, 32),
            torch.nn.Tanh(),
            torch.nn.Linear(32, self.layers + 1),
            torch.nn.Softmax(dim=1)  # 保证权重和为1，能量守恒
        )
    ...
    def forward(self, perturbed=False):
        ...
        # 存储所有层的 Embedding: [E0, E1, ..., EL]
        all_layer_embs = [ego_embeddings]

        # 2. GCN 传播 (LightGCN Backbone)
        # 低通滤波过程: A^k * X
        for _ in range(self.layers):
            ...
            if perturbed:
                ...
                ego_embeddings = ego_embeddings + torch.sign(ego_embeddings) * F.normalize(random_noise,
                                                                                           dim=1) * self.eps
            all_layer_embs.append(ego_embeddings)

        # 堆叠: [N, L+1, D]
        # N=Nodes, L=Layers, D=Emb_Size
        stack_embeddings = torch.stack(all_layer_embs, dim=1)

        # === 3. 核心创新: 频谱调制 (Spectrum Modulation) ===

        # (a) 计算 User 的个性化滤波器权重
        # 逻辑: 喜欢冷门的用户 -> gate 输出浅层权重高 (保留高频细节)
        #       喜欢热门的用户 -> gate 输出深层权重高 (利用低通平滑)
        user_embs_0 = self.user_embeddings.weight
        user_alpha = self.spectrum_gate(user_embs_0)  # [num_users, L+1]

        # (b) 计算 Item 的权重 (使用平均权重以保持稳健性)
        # 相当于标准的 LightGCN 处理方式
        num_layers = self.layers + 1
        item_alpha = torch.ones(self.num_items, num_layers).to(self.device) / num_layers

        # (c) 拼接全图权重
        # all_alpha: [num_users + num_items, L+1]
        all_alpha = torch.cat([user_alpha, item_alpha], dim=0)

        # (d) 加权融合
        # stack_embeddings: [N, L+1, D]
        # all_alpha:        [N, L+1] -> [N, L+1, 1]
        # 结果:              [N, D]
        final_embeddings = torch.sum(stack_embeddings * all_alpha.unsqueeze(-1), dim=1)
    ...
    def bpr_loss(self, user_emb, pos_emb, neg_emb, pos_idx, neg_idx):
        # === 这里的逻辑保持你原有的 gexingdu 版本不变 ===
        # 这确保了 "Loss 端" 依然在做显式的流行度匹配
    ...
    def cl_loss(self, u_idx, i_idx, j_idx):
        ...
        # 注意: CL loss 也使用了新的 forward，因此也享受了 PGSM 的增强
    ...
    def test_forward(self):
        # 专门用于测试的前向传播
        return self.forward(perturbed=False)


def test(model):
    user_embedding, item_embedding = model.test_forward()
