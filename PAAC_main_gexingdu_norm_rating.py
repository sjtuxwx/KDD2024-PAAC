...
class PAAC(torch.nn.Module):
    ...
    def bpr_loss(self, user_emb, pos_emb, neg_emb, pos_idx, neg_idx):
        ...
        user_emb = 0.5 * F.normalize(user_emb, dim=1)
        pos_emb = 0.5 * F.normalize(pos_emb, dim=1)
        neg_emb = 0.5 * F.normalize(neg_emb, dim=1)
...
def test(model):
    ...
    user_embedding = 0.5 * F.normalize(user_embedding, dim=1)
    item_embedding = 0.5 * F.normalize(item_embedding, dim=1)
                        ...
                        for gamma in ast.literal_eval(config.gama_list):
                            for tau in ast.literal_eval(config.tau_list):
                                for pop_gamma in ast.literal_eval(config.pop_gamma_list):
                                    ...
                                    f.write("PAAC_main_gexingdu_norm_rating.py")
