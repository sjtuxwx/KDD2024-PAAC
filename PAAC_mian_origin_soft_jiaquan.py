...
def main_args():
    ...
    # Added for compatibility with other scripts
...
class PAAC(torch.nn.Module):
    def __init__(self, config, data):
        ...
        # Register pop_count_tensor and max_pop for Soft Weighting
    ...
    def SoftInfoNCE(self, view1, view2, pop_weights, temperature):
        """
        Soft-weighted InfoNCE loss.
        pop_weights: [Batch, Batch] weight matrix for negative samples.
        """
        ...
        e_ttl = torch.exp(ttl / temperature)
        
        # Apply Soft Weights to negative samples
        # Note: We usually keep the positive pair (diagonal) weight as 1 or separate
        # Here we apply weights to all, assuming pop_weights diagonal is handled or irrelevant if pos is separate
        # But standard InfoNCE denominator includes positive pair.
        # Let's apply weights to e_ttl.
        
        weighted_e_ttl = e_ttl * pop_weights
        denom = weighted_e_ttl.sum(dim=1)
        
        # InfoNCE: -log( e_pos / denom )
        ratio = e_pos / (denom + 1e-8)
        cl_loss = -torch.log(ratio + 1e-8)
        return torch.mean(cl_loss)

    def cl_loss(self, u_idx, i_idx, j_idx):
        # Soft Weighting CL Loss
        
        # 1. Prepare Embeddings
        # We need all unique items in the batch to form the contrastive pool
        # But typically LightGCN CL uses the users/items in the current batch
        
        # Unique users and items in this batch
        batch_users = torch.unique(torch.tensor(u_idx)).type(torch.long).to(self.device)
        batch_items = torch.unique(torch.tensor(i_idx)).type(torch.long).to(self.device)
        
        # Augmentation Views
        user_view_1, item_view_1 = self.forward(perturbed=True)
        user_view_2, item_view_2 = self.forward(perturbed=True)
        
        # User CL (Standard InfoNCE, no pop weighting usually)
        user_cl_loss = metrics.InfoNCE(
            user_view_1[batch_users], user_view_2[batch_users], self.temperature) * self.cl_rate
            
        # Item CL (Soft Weighted by Popularity)
        # 1. Get Popularity for batch items
        # [Batch_Items]
        batch_pop = self.pop_count_tensor[batch_items].float()
        
        # 2. Calculate Pairwise Popularity Difference Matrix
        # [Batch_Items, 1] - [1, Batch_Items] -> [Batch_Items, Batch_Items]
        # We want to up-weight negative samples that have different popularity (Disentanglement)
        # Or up-weight popular negatives (Debiasing)
        
        # Strategy: Weight = gamma + (1 - gamma) * (1 - Normalized_Pop_Diff)
        # Reason: Mimic the Hard Split logic which worked well (gamma=0.2).
        # Hard Split Logic:
        #   - Same Group (Diff small): Weight = 1.0
        #   - Cross Group (Diff large): Weight = gamma (e.g. 0.2)
        #
        # Soft Logic:
        #   - Diff=0 (Same popularity) -> Weight = 1.0
        #   - Diff=1 (Max difference)  -> Weight = gamma
        
        pop_diff = torch.abs(batch_pop.unsqueeze(0) - batch_pop.unsqueeze(1))
        # Normalize by max pop in global or batch
        pop_diff_norm = pop_diff / (self.max_pop + 1e-8)
        
        # Weight Matrix
        # Linear interpolation between 1.0 (Same Pop) and gamma (Max Diff)
        pop_weights = self.gamma + (1.0 - self.gamma) * (1.0 - pop_diff_norm)
        
        # Calculate Item CL
        item_cl_loss_val = self.SoftInfoNCE(
        ...
        item_cl_loss = item_cl_loss_val * self.cl_rate
...
def train(config, data, model, optimizer, early_stopping, logger, train_step=1):
    ...
    for epoch in range(config.num_epoch):
        ...
        logger.info('len(val_U2I)=%d' % len(data.val_U2I))
        logger.info('len(test_U2I)=%d' % len(data.test_U2I))
                    ...
                    for lambda2 in ast.literal_eval(config.lambada_list):
                        for gamma in ast.literal_eval(config.gama_list):
                            ...
                            val_hr, val_recall, val_ndcg, test_OOD_hr, test_OOD_recall, test_OOD_ndcg, test_IID_hr, test_IID_recall, test_IID_ndcg, result_path = main(
                                config)
                            f.write('\n')
                            f.write("PAAC_main_origin_jiaquan")
                            f.write(
